...
$.fn.click_zan_block = function() {
	return this.each(function() {
		...
		$(this)
			.append(up_node)
			.append(down_node)
			.append($('<span class="uoj-click-zan-cnt">[<strong>' + (cnt > 0 ? '+' + cnt : cnt) + 's</strong>]</span>'));
	});
}
...
function showStandings() {
	$("#standings").long_table(
		...
		function(row) {
			...
			col_tr += '<td>' + '<div><span class="uoj-score" data-max="' + problems.length * 100 + '" style="color:' + getColOfScore(row[0] / problems.length) + '">+' + row[0] + 's</span></div>' + '<div>' + getPenaltyTimeStr(row[1]) + '</div></td>';
			for (var i = 0; i < problems.length; i++) {
				col_tr += '<td>';
				col = score[row[2][0]][i];
				if (col != undefined) {
					col_tr += '<div><a href="/submission/' + col[2] + '" class="uoj-score" style="color:' + getColOfScore(col[0]) + '">+' + col[0] + 's</a></div>';
